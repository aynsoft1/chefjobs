...
include_once(PATH_TO_MAIN_ADMIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_ADMIN1_ADMIN_MAILS);
...
if(tep_not_null($action))
{
 switch($action)
 {
  case 'delete':
    ...
    tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,$page_string. tep_get_all_get_params(array('lower','higher'))));
    break;
  ...
  case 'unmarked':
   ...
   tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,'rID='.$mail['sender_id']));
	  break;
 }
}
...
if(!$row_check=getAnyTableWhereData(ADMIN_EMPLOYER_MAILS_TABLE." as em "," em.id ='".(int)$mail_id."'","em.subject,em.attachment_file,em.message"))
{
 ...
 tep_redirect(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS);
}
if(($_GET['action'] == 'send_mail') && (tep_not_null($_GET['query_string1'])))
{
 ...
 $template->assign_vars(array(
  ...
  'INFO_TEXT_MESSAGE1'   => tep_draw_textarea_field('TR_message', 'soft', '60%', '10', '', '', true, true),
		'form'                 => tep_draw_form('send', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,'','post', 'onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('id',$mail_id).tep_draw_hidden_field('action1','confirm_send'),
		'button'               => tep_draw_submit_button_field('','Send','class="btn btn-primary"').'&nbsp;&nbsp;<a href="'.tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,'query_string1='.$_GET['query_string1']).'">'.tep_button('Back','class="btn btn-secondary"').'</a>',
...
// $file_directory=get_file_directory($row_check['attachment_file']);
 ...
 tep_db_query("update ".ADMIN_EMPLOYER_MAILS_TABLE." set receiver_see='Yes' where id='".$mail_id."'");
  ...
  'INFO_TEXT_MESSAGE1'=>stripslashes($row_check['message']),
		'INFO_TEXT_REPLY'   => "<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,"query_string1=".$_GET['query_string1'])."&action=send_mail'>".tep_button('Reply','class="btn btn-primary"')."</a>",
...
else
{
 ...
 $whereClause=" em.receiver_id =0 and em.sender_id='".$_GET['rID']."' ";
 ...
 if($x > 0 && $x1 > 0)
 {
  ...
  while ($jobseeker_mail = tep_db_fetch_array($result))
  {
   ...
   'subject'   => "<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,"query_string1=".$query_string1)."'>".tep_db_output($jobseeker_mail['subject'])."</a>",
 ...
 $template->assign_vars(array(
  ...
  'form'=>tep_draw_form('page', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_MAILS,'rID='.$rID,'post', 'onsubmit="return ValidateForm(this)"'),
  'view_link'   => $view_link,
