...
include_once(PATH_TO_MAIN_ADMIN_PHYSICAL_LANGUAGE . $language . '/' . FILENAME_QUIZ_MESSAGES);
...
function getFormTag($actionValue, $dataID = null)
{
    switch ($actionValue) {
        case 'new':
            return tep_draw_form('question formElement', PATH_TO_ADMIN . FILENAME_QUIZ_MESSAGES, 'action=submitForm', 'post', 'id="formElement" enctype="multipart/form-data"');
            break;
        case 'edit':
            return tep_draw_form('question formElement', PATH_TO_ADMIN . FILENAME_QUIZ_MESSAGES, 'message_id=' . $dataID . '&action=updateForm', 'post', 'id="formElement" enctype="multipart/form-data"');
            break;
    }
}
...
function getAction(int $messageId, int $quizId)
{
    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu">
            <a class="dropdown-item" href="' . tep_href_link(PATH_TO_ADMIN . FILENAME_QUIZ_MESSAGES, 'id=' . $quizId . '&message_id='.$messageId.'&action=edit') . '" name="editBtn" id="editBtn-'.$messageId.'">
                ' . EDIT_TEXT . '
            </a>
            <a class="dropdown-item" href="' . tep_href_link(PATH_TO_ADMIN . FILENAME_QUIZ_MESSAGES, 'message_id=' . $messageId . '&action=confirm_delete&id='.$quizId) . '" 
                name="deleteBtn" 
    ...
    switch ($action) {
        case 'submitForm':
            ...
            if (!$error) {
                ...
                return tep_redirect(FILENAME_QUIZ_MESSAGES . '?id=' . $input_quiz_id . '&action=listMessage');
            }
            break;
        case 'updateForm':
            if (!$error) {
        ...
        case 'confirm_delete':
            ...
            tep_redirect(FILENAME_QUIZ_MESSAGES . '?id=' . $quiz_id . '&action=listMessage');
            break;
    }
}
    ...
    $template->assign_vars(array(
        ...
        'new_button' => '
            <a 
                ...
                href="' . tep_href_link(PATH_TO_ADMIN . FILENAME_QUIZ_MESSAGES, 'action=new&id=' . $quiz_id) . '">
