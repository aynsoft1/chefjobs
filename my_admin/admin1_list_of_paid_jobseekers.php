...
if($db_jobseeker_num_row > 0)
{
 ...
 while ($jobseeker = tep_db_fetch_array($db_jobseeker_query))
 {
  if($action1!='delete')
  {
   ...
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_INACTIVE, 28, 22) . '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_ACTIVATE, 28, 22) . '</a>';
   }
   if ($jobseeker['jobseeker_featured'] == 'Yes')
   {
    $featured_status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_not_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOBSEEKER_NOT_FEATURED, 28, 22) . '</a>' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOBSEEKER_FEATURED, 28, 22);
   }
   else
   {
    $featured_status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_NOT_FEATURE, 28, 22) . '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_FEATURED, 28, 22) . '</a>';
   }
  }
  else
  {
...
{
 case 'delete':
  $heading[] = array('text' => '<div class="list-group"><h4 class="mb-0">' . $rInfo->jobseeker_name . '</div>
...
{
 $check_link='<a href="#" onclick="checkall()">Check All</a> / <a class="me-3" href="#"  onclick="uncheckall()">Uncheck All</a><a class="text-danger" href="#"  onclick="DeleteSelected(\'delete\')"><i class="bi bi-trash"></i></a></font>';
 $template->assign_vars(array(
...
'TABLE_HEADING_ACTION'=>TABLE_HEADING_ACTION,
 ...
 'new_button'=>'<a class="btn btn-primary" href="' . tep_href_link(FILENAME_JOBSEEKER_REGISTER1, 'add=jobseeker') . '"><i class="bi bi-plus-lg me-2"></i>Add New</a>',
  ));
}
