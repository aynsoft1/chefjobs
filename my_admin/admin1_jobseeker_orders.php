...
$template->set_filenames(array('order' => 'admin1_jobseeker_orders.htm','order1' => 'admin1_jobseeker_orders1.htm','email' => 'jobseeker_ord_status_update_tmpl.htm','invoice'=>'admin1_jobseeker_invoices.htm'));
	...
	switch ($action)
	{
		case 'invoice_mail':
		 ...
		 include_once(PATH_TO_MAIN_PHYSICAL_CLASS . 'jobseeker_order.php');
          ...
          $customer_name =tep_db_output($order->customer['name']);
          $customer_email_address ='sp_patnaik2003@yahoo.co.in';//tep_db_output($order->customer['email_address']);
           $credit_card_string='';
		   ...
		   $order_total_text.='
			...
			$template->assign_vars(array(
				...
				'customer_email_address'=>$customer_email_address,
 				'INFO_TEXT_INVOICE'=>$INFO_TEXT_INVOICE,
			...
			$email_text =$template->pparse1('invoice');
 		} 
		tep_mail($customer_name, $customer_email_address,  EMAIL_TEXT_SUBJECT_INVOICE.$oID, ($email_text), SITE_OWNER, EMIAL_FROM);
    	$messageStack->add_session(MESSAGE_INVOICE_SENT, 'Success');
 ...
 $alternate=1;
	while ($orders = tep_db_fetch_array($orders_query))
	{
		...
		if (isset($oInfo) && is_object($oInfo) && ($orders['orders_id'] == $oInfo->orders_id))
		{
			...
			$row_selected=' id="defaultSelected" class="table-secondary dataTableRowSelected" onmouseover="rowOverEffect(this)"';
   $action_image=tep_image(PATH_TO_IMAGE.'icon_arrow_right.gif',IMAGE_EDIT);
	...
	$email_invoice = '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('iID', 'action','selected_box')) . 'iID=' . $orders['orders_id'] . '&action=invoice_mail') . '"><i class="fa fa-envelope" style="font-size:16px"></i></a>';
  //print_r($orders);die();
  $alternate++;
