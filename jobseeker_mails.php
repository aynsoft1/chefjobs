...
if(!$row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," (a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' OR ai.receiver_id = '".$_SESSION['sess_jobseekerid']."') and  ai.id ='".(int)$mail_id."'","ai.subject,ai.attachment_file,ai.message,ai.sender_user"))
{
  print_r($row_check);exit;
...
elseif($mail_id>0)
{
 ...
 tep_db_query("update ".APPLICANT_INTERACTION_TABLE." set user_see='Yes' where id='".$mail_id."'");
  ...
  'INFO_TEXT_BACK'=>'<a href="'.FILENAME_JOBSEEKER_MAILS.'"><button class="btn btn-outline-secondary me-2"><i class="bi bi-arrow-left"></i></button>',
...
else
{
 ...
 if($show_mail_status=='Inbox')
  $view_link1 =INFO_TEXT_VIEW_IN."<a class='text-blue' href='#'  onclick='view_data(\"All\",\"Sent\")'>".INFO_TEXT_VIEW_SEND."</a> | <a href='#' class='red'  onclick='view_data(\"All\",\"Inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>";
 elseif($show_mail_status=='Send')
  $view_link1 =INFO_TEXT_VIEW_IN."<a class='text-blue' href='#'  onclick='view_data(\"All\",\"Active\")'>".INFO_TEXT_VIEW_INBOX."</a> | <a href='#' class='red'  onclick='view_data(\"All\",\"Inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>";
 else
  $view_link1 =INFO_TEXT_VIEW_IN. "<a class='text-blue' href='#'  onclick='view_data(\"All\",\"Send\")'>".INFO_TEXT_VIEW_SEND."</a> | <a class='text-blue' href='#'  onclick='view_data(\"All\",\"Active\")'>".INFO_TEXT_VIEW_INBOX."</a>";
...
//  $whereClause=" a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."'";
 $whereClause = " (a.jobseeker_id = '" . $_SESSION['sess_jobseekerid'] . "' OR ai.receiver_id = '" . $_SESSION['sess_jobseekerid'] . "')";
 //print_r($_POST);
