 ...
 function cart()
 {
  ...
  else
  {
    if(CONF_ORDER_TAX>0)
    {
	  $tax =($products['job_posting_total_fee']*CONF_ORDER_TAX)/100;
      $final_total_fee  =$products['job_posting_total_fee'] +$tax;
      $this->info['subtotal_symbol'].="<br>Tax :".$currencies->format($tax, ($products['currency']!=DEFAULT_CURRENCY?true:false), DEFAULT_CURRENCY, ($products['currency']==DEFAULT_CURRENCY?$currencies->get_value($products['currency']):''))."<br>";
