...
if ($action=='check') 
{
	...
	$recruiter_password=$_POST['password'];
	$error =false;
	$reg =false;
	$errorMsg=array();
	...
	if(!tep_not_null($recruiter_password))
	{
 	 $error =true;
     $errorMsg [] ='invalid password';
	}
    elseif(strlen($recruiter_password) < MIN_PASSWORD_LENGTH) 
	{
 	 $error =true;
	...
	if(!$error)
	{
 	$whereClause="rl.recruiter_email_address='".tep_db_input($email_address)."' and rl.recruiter_id=r.recruiter_id";
    $fields='rl.recruiter_id,concat(r.recruiter_first_name," ",r.recruiter_last_name) as name,rl.recruiter_email_address,rl.recruiter_password,rl.ip_address,rl.number_of_logon,rl.recruiter_status';
	if($row=getAnyTableWhereData(RECRUITER_LOGIN_TABLE.' as rl left outer join  '.RECRUITER_TABLE.' as r on (rl.recruiter_id=r.recruiter_id) ',$whereClause,$fields,false))
	{
	 $reg =true;
     if($row['recruiter_status']!='Yes')
	 {
   	  $error =true;
   ...
   session_start();
      ...
      //$_SESSION['sess_jobseekerid']=$row["recruiter_id"];
	  $_SESSION['language']=$language;
	  $_SESSION['languages_id']=$language_id;
	  $_SESSION['sess_access_key']= get_access_key($row['recruiter_id'],$ip_address);
      tep_redirect(tep_href_link('app/recruiter_info/'));
     }
	}
	else
	{
