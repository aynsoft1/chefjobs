...
//tep_redirect(tep_href_link(FILENAME_SUBSCRIPTION_ERROR));
 ...
 {
  if(!$edit){
  $messageStack->add_session(WAIT_CONFIRMATION, 'Plan Expired');
  tep_redirect(tep_href_link(FILENAME_RECRUITER_RATES));
  }
 }
		/////Error code/////////
...
else
{
 ...
 {
  //$buttons=tep_image_submit(PATH_TO_BUTTON.'button_preview.gif', IMAGE_PREVIEW, 'name="Preview"').'&nbsp;&nbsp;<a href="'.tep_href_link(FILENAME_RECRUITER_CONTROL_PANEL).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>';
   $buttons=tep_draw_submit_button_field('Preview',''.INFO_BUTTON_PREVIEW.'','class="btn btn-primary"');//tep_image_submit(PATH_TO_BUTTON.'button_preview.gif', IMAGE_PREVIEW, 'name="Preview"');
 ...
 if($state_error)
 {
  ...
  {
   define('INFO_TEXT_STATE1',LIST_SET_DATA(ZONES_TABLE,"",TEXT_LANGUAGE.'zone_name','zone_id',TEXT_LANGUAGE."zone_name",'name="state" id="state" class="form-select"',INFO_TEXT_STATE,'',$state_value));
  }
  else
  {
   define('INFO_TEXT_STATE1',LIST_SET_DATA(ZONES_TABLE,"",TEXT_LANGUAGE.'zone_name','zone_id',TEXT_LANGUAGE."zone_name",'name="state" id="state" class="form-select"',INFO_TEXT_STATE,'',$state_value));
...
/**** calling company logo and name for preview job****/
 if($comp_det=getAnytableWhereData(RECRUITER_TABLE,"recruiter_id='".$_SESSION['sess_recruiterid']."'",'recruiter_company_name,recruiter_logo'))
...
else
{
 $template->assign_vars(array(
  ...
  'INFO_TEXT_LOCATION'=>INFO_TEXT_LOCATION,
  'INFO_TEXT_LOCATION1'=>get_city_dropdown_list($state_value, 'name="location" id="location" class="form-select"', "City", "", $location),
  'INFO_TEXT_SALARY'=>INFO_TEXT_SALARY,
  ...
  'INFO_TEXT_INDUSTRY_SECTOR'=>INFO_TEXT_INDUSTRY_SECTOR,
  'INFO_TEXT_INDUSTRY_SECTOR1'=>get_drop_down_list(JOB_CATEGORY_TABLE,"name='TR_post_job_category[]' class='form-control form-select required multiple' onchange='fetchJobSubCategories(this)'","".INFO_TEXT_INDUSTRY_SELECT."","0",$post_job_category),
...
'INFO_TEXT_PROFESSION_SECTOR'=>INFO_TEXT_PROFESSION_SECTOR,
'INFO_TEXT_INDUSTRY_SUB_SECTOR'=>get_sub_category_drop_down_list(JOB_SUB_CATEGORY_TABLE,"name='TR_post_job_sub_category' id='TR_post_job_sub_category' class='form-control form-select required'","".INFO_TEXT_PROFESSION_SELECT."","0",$job_sub_category),
  ...
  'INFO_COMPANY_SIZES'=>INFO_COMPANY_SIZES,
  'INFO_COMPANY_SIZES1'=>LIST_TABLE(COMPANY_SIZE_TABLE, TEXT_LANGUAGE.'size_name',"",'name="company_sizes" class="form-select"',"".INFO_TEXT_COMPANY_SIZE."",'',''.$company_sizes.''),
  'INFO_CAREER_LEVEL'=>INFO_CAREER_LEVEL,
  'INFO_CAREER_LEVEL1'=>LIST_TABLE(CAREER_LEVEL, TEXT_LANGUAGE.'career_level_name',"",'name="career_level" class="form-select"',"".INFO_TEXT_CAREER_LEVEL."",'',''.$career_level.''),
