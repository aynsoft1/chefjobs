 ...
 if(tep_db_num_rows($jobfair_query) > 0)
 {
  ...
  while ($jobfair = tep_db_fetch_array($jobfair_query))
  {
   ...
   $jobfair_seo_name = $jobfair["jobfair_seo_name"].'-jobfair';
	$jf_short_desc=((strlen($jobfair['jobfair_short_description'])<100)?$jobfair['jobfair_short_description']:substr($jobfair['jobfair_short_description'],0,100)."..");
   ...
   'jobfair_name' =>'<a class="" href="'.$jobfair_url.'">'.tep_db_output($jobfair['jobfair_title']).'</a>',
...
if(tep_not_null($row_check["jobfair_logo"]) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_JOBFAIR_LOGO.$row_check["jobfair_logo"]))
  $jobfair_logo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_JOBFAIR_LOGO.$row_check["jobfair_logo"].'','','','','class="extra-large-profile-img me-4"');
///******replacing youtube link with embed code****/////////////////
 ...
 $embed_code = preg_replace($search_video,$replace_video,$jobfair_video_link);//preg_replace("/watch\?v=/i", 'embed/', $link);
  $jobfair_video='<iframe width="100%" height="550" src="'.$embed_code.'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
///////***************/////////////////////
$jobfair_googlemap_url='<iframe style="width: 100%;" src="'.$row_check["jobfair_googlemap_url"].'" width="100%" height="400" frameborder="0" style="border:0" allowfullscreen></iframe>';
...
{
	 $company_logo="<a href='".tep_href_link(FILENAME_JOBFAIR_JOBS,'query_string='.$query_string.'&jfid='.$jobfair_id)."' class='blue'>".tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_LOGO.$company_logo."&size=100")."</a>";
...
///-------------------------------------------////
 ...
 $query_string=encode_string("recruiter_id=".$recruiter_id."=recruiter_id");
   ...
   $company_name="<a href='".tep_href_link(FILENAME_JOBFAIR_JOBS,'query_string='.$query_string.'&jfid='.$jobfair_id)."' class='text-blue'>".tep_db_output($row['recruiter_company_name'])."</a> ";

   $job_title='<a href="'.getPermalink('job',array('ide'=>$job_id,'seo_name'=>$title_format)) .'" target="_blank" class="">'.tep_db_output($row['job_title'])."</a> ";
...
$job_display.='	<div class="col-md-3 mb-4">
				...
				<div class="card-body">
				  ...
				  <h4 class="mb-2">'.$job_title.'</h4>
				  <div class=""><i class="bi bi-building"></i> '.$company_name.'</div>
				  <div class=""><i class="bi bi-geo-alt"></i> '.$job_location.'</div>
				  <div class="exp">'.$job_experience.'</div>
				  <div class=""><i class="bi bi-currency-dollar"></i>'.tep_db_output($row['job_salary']).'</div>
...
{
 $template->assign_vars(array(
   'sort_jobfair'=>tep_draw_pull_down_menu('sort', $jobfair_view_array, $_GET['sort'],'onchange="document.form1.submit();" class="form-select" style="width:175px;"'),
