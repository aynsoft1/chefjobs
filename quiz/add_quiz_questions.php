...
include_once(PATH_TO_MAIN_QUIZ_PHYSICAL_LANGUAGE . $language . '/' . FILENAME_RECRUITER_LIST_OF_QUESTIONS);
...
if (tep_not_null($question_id)) {
    if (!$row_check_question_id = getAnyTableWhereData(QUES_TABLE, "id='" . tep_db_input($question_id) . "'")) {
...
function getQuestionAction(int $quesId, int $quiz_id)
{
    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu">
            <a class="dropdown-item" href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . $quiz_id . '&id=' . $quesId . '&action=edit') . '">
                ' . EDIT_TEXT . '
            </a>
            <a class="dropdown-item" href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . $quiz_id . '&id=' . $quesId . '&action=confirm_delete') . '">'
        . DELETE_TEXT . '
            </a>
        </div>
    </div>
...
{
    $button = '
        <a class="btn btn-primary" href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . $quiz_id . '&action=allQuestion') . '">
            ' . ADD_QUESTION . '
        </a>
    ';
    return $button;
...
function getFormTag($actionValue, $dataID = null)
{
    switch ($actionValue) {
        case 'new':
            return tep_draw_form('question formElement', PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'action=submitForm', 'post', 'id="formElement" enctype="multipart/form-data"');
            break;
        case 'edit':
            return tep_draw_form('question formElement', PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'id=' . $dataID . '&action=updateForm', 'post', 'id="formElement" enctype="multipart/form-data"');
            break;
    }
}
    ...
    switch ($action) {
        case 'submitForm':
            ...
            if (!$error) {
                ...
                return tep_redirect(FILENAME_RECRUITER_LIST_OF_QUESTIONS . '?quiz_id=' . $question_quiz_id . '&action=allQuestion');
            }
            break;
        case 'updateForm':
            validationFormCheck([
        ...
        case 'confirm_delete':
            if (!$error) {
                ...
                tep_redirect(FILENAME_RECRUITER_LIST_OF_QUESTIONS . '?quiz_id=' . $quiz_id . '&action=allQuestion');
            }
            break;
    }
}
...
if ($action === 'allQuestion' && tep_not_null($quiz_id)) {
    $fetch_all_questions = "SELECT * FROM " . QUES_TABLE . " as ques WHERE ques.isActive = '1' AND ques.deleted_at IS NULL AND ques.quiz_id = " . $quiz_id . " ORDER BY ques.created_at DESC";
    ...
    $fetch_all_quiz_query = "SELECT * FROM " . QUIZ_TABLE . " as quiz WHERE quiz.isActive = '1' AND recruiter_id=$recruiter_id ORDER BY quiz.created_at DESC";
        ...
        while ($quiz = tep_db_fetch_array($get_all_quiz)) {
            ...
            $template->assign_block_vars('quizs', array(
                ...
                'title' => '<a href="' . tep_href_link(PATH_TO_QUIZ . $quiz['id'] . '/' . encode_forum($quiz['title']) . '.html') . '" target="_blank" rel="noreferrer">
                    ...
                    '</a>',
                'countQuestions' => '<a href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . tep_db_output($quiz['id']) . '&action=allQuestion') . '">
                                        ' . countQuizHasQuestions(tep_db_output($quiz['id'])) .
                    '</a>',
                'created_at' => tep_date_short($quiz['created_at']),
                // 'action' => getAddQuestionAction(tep_db_output($quiz['id'])),
                'action' => '<a class="btn btn-primary" 
                                href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . tep_db_output($quiz['id']) . '&action=new') . '">
...
if ($action == 'allQuestion' && tep_not_null($quiz_id)) {
    ...
    $template->assign_vars(array(
        ...
        <a 
            ...
            href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_LIST_OF_QUESTIONS, 'quiz_id=' . $quiz_id . '&action=new') . '">
            <i class="fa fa-plus" aria-hidden="true"></i> ' . ADD_TEXT . '
        </a>
    ',
    ));
    ...
    $template->assign_vars(array(
        ...
        'TABLE_HEADING_TOTAL_QUESTION' => TABLE_HEADING_TOTAL_QUESTION,
