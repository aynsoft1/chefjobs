...
{
    $query = "SELECT COUNT(an.id) AS total FROM `jobseeker_notifications` AS an WHERE an.is_read = '0'";
...
{
    $query = "UPDATE jobseeker_notifications SET is_read = 1 WHERE id IN (SELECT id FROM (SELECT id FROM jobseeker_notifications WHERE is_read = 0) as temp)";
    $res = tep_db_query($query);
    return $res;
}
