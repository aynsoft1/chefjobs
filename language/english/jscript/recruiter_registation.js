...
function ValidateEMail(objName) {
  ...
  if (iobjLength != 0) {
    if (iFposition == -1 || iSposition == -1) {
      ...
      iobjLength - (iTmp + 1) < 2 ||
      (iobjLength - (iTmp + 1) > 3 && sobjValue.slice(iTmp) != ".technology")
    ) {
      return false;
...
function ValidateForm(objFrm) {
  ...
  for (var i = 0; i < objFrm.length; i++) {
    ...
    if (
      ...
      // Adding validation for city field
      if (objFrm[i].name == "city") {
        var cityValue = objFrm[i].value.trim();
        if (cityValue === "") {
          alert("Please enter a city/town.");
          objFrm[i].focus();
          return false;
        } else if (cityValue.length < 3) {
          alert("City/town must be at least 3 characters long.");
          objFrm[i].focus();
          return false;
        }
      }
      ...
      //   const errorMap = ["Invalid number", "Invalid country code", "Telephone number is too short", "Telephone number is too long", "Invalid telephone number "];
